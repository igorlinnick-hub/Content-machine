...
- **Не закоммичено — клипы:** `lib/clips/ffmpeg.ts` (замер реального FPS) и два роута,
...
4. Завести 12 эстетических триггеров в ManyChat HWC — **BOTOX уже напечатан на живом посте**,
   до этого бот на него молчит.
5. Дописать `scripts/clips-runner` по образцу `canva-runner` и только тогда коммитить клипы.
6. Дождаться первого штатного крона floor-media (07:00 UTC) и проверить пуш-дайджест.
