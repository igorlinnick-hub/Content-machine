...
| 11 | Телепромптер → авточистка: кнопка «✂️ Auto-edit» на экране Saved (один тап, стиль клиники уже выбран) | ✅ 2026-07-02 | До этого записи телепромптера уезжали в `Recordings/{клиника}` и пайплайн их НЕ видел. Теперь: `POST /api/clips/from-recording {recordingId}` — копирует файл в Inbox клиники (`copyFileToInbox`, оригинал в Recordings нетронут) → синхронно `processClip` (maxDuration 300). Self-healing: оборвался запрос — копия уже в Inbox, cron доделает за ≤30 мин. UI: running/done/error states, ссылки Final + Video editor. Kill switch уважается (`disabledHttpResponse`) |
