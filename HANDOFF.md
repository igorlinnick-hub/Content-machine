...
10. **Telegram убран из продукта полностью** (подтверждено Игорем 2026-07-06; бот-слой lib/team вычищен ранее, остатки plumbing — 2026-07-06). Врач и команда работают через веб (телепромптер, /clips, back-office). Никакой новый код не должен опираться на TG; уведомления — через web push / email / in-app (канал не выбран, см. Track 1 п.9).
...
| 9 | Уведомление врачу/команде «клип готов» | ✅ 2026-07-06 (web push + in-app бейдж) | **Telegram УБРАН из продукта (Игорь, 2026-07-06)** — TG-вариант был написан и в тот же день откачен; мёртвый Pax-plumbing (`triggered_chat_id`) вычищен из кода (колонка в DB осталась, дроп после деплоя). Выбранные каналы (Игорь, 2026-07-06): **(a) Web push** — `public/sw.js` + `lib/push/send.ts` (web-push npm, VAPID; 404/410 → подписка удаляется) + Migration 039 `push_subscriptions` + `POST/DELETE /api/push/subscribe` + `app/components/PushToggle.tsx` (кнопка 🔔 на экране Saved телепромптера + compact в /clips; iOS без PWA-установки → подсказка Add to Home Screen). Хуки в cron + from-recording, best-effort. **(b) In-app бейдж** — карточка «Video editor» в DashBento (`/clips`) с pill «N new» (`NewClipsBadge` + `/api/clips/summary`; seen-маркер = localStorage, ставится `MarkClipsSeen` при заходе на /clips; первый визит = окно 7 дней). Env: `NEXT_PUBLIC_VAPID_PUBLIC_KEY`, `VAPID_PRIVATE_KEY`, `VAPID_SUBJECT` (mailto:) — не заданы → push молча выключен |
| 10 | Caption-стили пресетами: 4 визуальных шаблона субтитров на выбор | ✅ 2026-07-02 | `lib/clips/captionStyles.ts` — classic/clean/bold/pop (libass force_style); Migration 038 `clinics.caption_style` (default 'classic'); pipeline fail-open (нет колонки/ошибка → classic, старый вывод не меняется); выбор — в `/clips` (страница переименована в «Video editor», picker с CSS-превью, POST `/api/clips/style`). Стиль = default клиники, применяется ко ВСЕМ новым клипам (cron Inbox + Auto-edit) |
| 11 | Флоу монтажа v2 (решение Игоря 2026-07-07): врач ТОЛЬКО записывает, весь монтаж — в админ-секции | ✅ 2026-07-07 | **Врач:** экран Saved телепромптера = «🎬 The video editor has been notified» + Record another, никаких кнопок монтажа. Upload-recording шлёт push команде («🎬 New recording»). **Админ (/clips, admin-only redirect):** RecordingsPanel — скролл записей (clinic_recordings status=final), превью в приложении (Drive iframe `/preview`), «✂️ Auto-edit» на выбранной (POST `/api/clips/from-recording`, admin-only) и 🗑 удаление (DELETE `/api/studio/recordings` — файл удаляется и из Drive, чтобы не копить). Механика from-recording прежняя: копия в Inbox → processClip синхронно; self-healing через дневной cron. Карточка Video editor в DashBento — adminOnly. POST `/api/clips/style` — admin-only |

**Pre-deploy (пп.9-11):** накатить Migrations 038 (`clinics.caption_style`) + 039 (`push_subscriptions`) в Supabase SQL Editor. Vercel env: `NEXT_PUBLIC_VAPID_PUBLIC_KEY` + `VAPID_PRIVATE_KEY` + `VAPID_SUBJECT` (сгенерированы 2026-07-06, переданы Игорю в чате; можно перегенерить `npx web-push generate-vapid-keys` — тогда старые подписки умрут). Всё fail-open: без миграции 038 пайплайн жжёт classic; без VAPID env push молча выключен; бейдж работает сразу. Новая зависимость: `web-push` (+`@types/web-push` dev).

**Серверная цепочка монтажа — LIVE (2026-07-20).** Транскрибация = **Replicate WhisperX** (`victor-upmeet/whisperx-a40-large`, ~$0.01/мин, `align_output` для word-level; та же модель, что у локального video editing bot — `My Bots/bot/scripts/transcribe_replicate.py`); OpenAI Whisper остался fallback'ом, ключ НЕ нужен. Drive-записи клипов идут через user-OAuth клиент (SA не имеет квоты в личном Drive). `GOOGLE_DRIVE_CLINICS_ROOT_ID` = папка «Clinic Clips» (`1erhf5AURtETtyXUlnskCNiSVUvD_JfKJ`). Обе клиники запровижинены 2026-07-20 (Dr. Shawn `5065c6ee…`, Dr. Made `cdb530e5…`) через новый admin-роут `POST /api/clinics/provision-drive`. Миграция 037 накачена. `ENABLE_LLM_AGENTS=true` на проде (включён для монтажа — помнить про расход на генерации постов). Статические видео Игоря (captions-only, без b-роллов) — папка «Static Videos — Captions Only» + флоу-док `My Bots/bot/scripts/STATIC-VIDEOS.md`.

### 22.2b ПРАВИЛА МОНТАЖА talking-head (BINDING, создано 2026-07-23)

Источник: проверенная цепочка локального video bot (`My Bots/bot/scripts/process_video.py`) + официальные спеки Instagram Reels. Любое изменение пайплайна `lib/clips/*` обязано соблюдать эти правила; менять правила — только через этот раздел.

**1. Резка — мягкая, не дёрганая:**
- Паддинг **±0.2s** вокруг каждой фразы (бот выяснил: 0.1s режет слоги)
- Паузы **<0.5s не режутся вообще** — естественный ритм речи остаётся
- Паузы длиннее — вырезаются, но паддинги оставляют ~0.4s «дыхания» на стыке (никаких склеек встык)
- Фрагменты короче 0.4s отбрасываются; чистые филлеры (um/uh/er/hmm) — всегда
- Повторные дубли («ещё раз, сначала») — Sonnet-пасс (НЕ Haiku — Haiku слаба для решений об удалении контента), fail-open

**2. Кадр:**
- Выход **всегда 1080×1920 (9:16), 30fps CFR**. Никогда не letterbox («маленькое видео в чёрном море» — запрещено, Игорь 2026-07-23)
- Вертикальный исходник заполняет кадр как есть; горизонтальный — центр-кроп до 9:16 (лицо в вебкам-записи по центру)
- Лицо по центру кадра; face-tracking кроп — бэклог (пока строго геометрический центр)
- Правило для съёмки: врачу снимать **вертикально на телефон, ≥1080p**. Горизонтальная вебкамера 720p при кропе неизбежно апскейлится и мылит — это ограничение исходника, не пайплайна

**3. Экспорт (спеки Instagram: 1080×1920, H.264, 5-10 Mbps, AAC 256k/48kHz):**
- Финал: H.264 **High profile, CRF 18, preset fast**, yuv420p, faststart
- Аудио: **AAC 256 kbps, 48 kHz**
- **≤2 поколений кодирования**: normalize (intermediate CRF 17) → финальный проход «резка+субтитры» ОДНИМ энкодом (`cutAndBurn`). Раньше было 3 поколения CRF 23/veryfast — «пиксельная залупа», запрещено

**4. Субтитры:**
- Прожиг только на финальном проходе; шрифт бандловый (assets/fonts, Liberation Sans) через `fontsdir` — на Vercel нет системных шрифтов
- Кьюсы по фразам Whisper с ремапом на пост-монтажный таймлайн (`plan.cues`), НЕ по склеенным блокам
- Анимированные (karaoke): чанки ≤3 слов на строку (правило бота)

**5. Длительность:** цель ≤90s для Reels; длиннее — не резать силой, сигналить в UI (бэклог).
...
## 23. GO-TO-MARKET READINESS — «острая идея, которую легко продать» (создан 2026-07-06)

Стратегический раунд по продаваемости. Разбор пришёл от анализа референс-ролика (`../Marketing 01.MP4` — product-led demo агентского SaaS; транскрипт + разбор формата в `../Marketing 01 — transcript.md`). Кода в этом раунде не менялось — это план, который ведёт следующие сессии.

**Диагноз (что мешает продавать сейчас):**
1. Продукт **широкий, а не острый.** Умеет много (генерация + compliance + авто-монтаж + arsenal + studio + content-plan + Canva), но нет одного собранного экрана, проходящего путь «сырьё → опубликовано». Референс продаёт **одну вещь** — значит, нам нужен один демонстрируемый money-shot.
2. **Два покупателя в одной голове.** Клиника (white-label, high-ticket, defensible через compliance) и Studio (масс-маркет креаторы) — разная боль. Compliance масс-маркету безразличен. Одно сообщение на обоих = размыть оба.
3. **Нет входной двери.** `app/page.tsx` = форма логина. Холодному трафику показать нечего → масс-маркет не продать (некуда лить рекламу). Подтверждено: `app/(marketing)` не существует.
4. **Нет пруфа.** Покупают результат, а витрины «вот что сгенерила машина / before→after монтажа / что зашло» нет.
5. **Consumer не self-serve.** Подтверждено: Stripe/биллинга нет, `workspace_id` нет (Studio-движок привязан к `clinic_id`). Масс-маркет = самообслуживание + paywall с дня 1 (урок kill switch, §22.1 п.8).

### 23.1 Решения (зафиксировано 2026-07-06)

1. **Две острые идеи под двух покупателей, не одна на всех.** Продавать разными роликами/лендингами:
   - **Клиникам (бизнес, регулируемые ниши):** «Врач говорит в камеру 60 секунд → неделя комплаентных постов в его бренде, и **ничего не публикуется без прохождения медицинской проверки**.» Защищаемо (generic-AI это не трогает), дорого, бьёт туда, где клиники получают иски.
   - **Масс-маркету (креаторы/коучи/малый бизнес = Studio):** «Говори в камеру → готовый к публикации клип + текст за минуты. Правишь, **удаляя слова, а не двигая таймлайн**.» Конкуренция за «что сказать + скорость чистки», НЕ за эффекты (там CapCut/Opus). Compliance не упоминается.
2. **Клиника — first (выручка + ров), Studio — second** (после первых денег). Согласуется с §22.1.
3. **Лендинг делать раньше швов продукта** (личный совет Claude, принято): публичная страница с одним обещанием + форма «записаться» позволяет тестировать сообщение на клиниках ДО допила флоу. Дешевле проверить сообщение на лендинге, чем в коде.
4. **Compliance — сделать видимым, а не спрятанным гейтом.** Бейдж «passed medical-compliance check» в UI. Это продающий аргумент №1 для клиник — сейчас невидим (`lib/agents/compliance.ts` работает, но в UI не выведен).
5. **Нарратив сворачиваем до одной геройской петли.** Arsenal / studio / lab / visual — «power features» под капотом, НЕ в сообщении. Одно демо = одна петля.
6. **ИИ-аватары — по-прежнему нет** (§22.6). Настоящее лицо = доверие.

### 23.2 Track A — Clinic GTM (revenue-first, делать первым)

| # | Процесс | Оценка | Файлы / где | Статус |
|---|---|---|---|---|
| 1 | **Публичный лендинг** (отдельно от логина) — hero = запись money-shot флоу, одно обещание (23.1 п.1 клиника), форма «записаться на демо» | 1-2 дня | новый route-group `app/(marketing)/page.tsx` (текущий `app/page.tsx` = логин, не трогать); ui-ux-pro-max + HWC бренд-система | ❌ |
| 2 | **Money-shot seam** — один сквозной визуальный проход `запись на телепромптере → тап Auto-edit → клип готов → карусель → в расписание`, без переключения контекста | 2-3 дня | сшить существующее: `app/teleprompter/*` → `app/api/clips/from-recording/route.ts` → `app/clips/page.tsx` → `app/api/posts/generate/route.ts` → `app/scheduler/*` → `app/api/scheduled-posts/route.ts` (постинг-конец уже реален: Buffer-адаптер `app/api/publish/buffer/route.ts`) | ❌ |
| 3 | **Compliance-бейдж видимым** — вывести grade PASS/REWORD/REVIEW из гейта в UI постов + витрину «прошло проверку» | 1 день | источник: `lib/agents/compliance.ts` (grade уже считается) → `app/visual/components/PostsWorkspace.tsx` / карточка поста | ❌ |
| 4 | **Витрина результата** `/showcase` — галерея реально сгенерённого + before→after авто-монтажа (пруф) | 1-2 дня | новый route + блок на лендинге; сид из `docs/sample-outputs.md` + реальные клипы | ❌ |
| 5 | **Аналитическая петля** — какой пост зашёл → назад в content-plan / few-shot (делает подписку липкой: «генерим то, что работает») | 3-5 дней (отложить до первых клиентов) | ingest метрик (ручной ввод/CSV на старте) → `lib/content-plan.ts` / few-shot; новая таблица | ❌ отложено |

### 23.3 Track B — Studio consumer self-serve (mass-market wedge, делать вторым)

Наследует детальные процессы из §22.3 — здесь только то, что критично именно для продаваемости масс-маркету. Порядок: движок → биллинг → лендинг Studio.

| # | Процесс | Оценка | Файлы / где | Статус |
|---|---|---|---|---|
| 1 | Развязать движок от `clinic_id` → `workspace_id` (подтверждено: `workspace_id` в коде нет) | 2-3 дня | `lib/studio/*`, `loadSharedContext`, DB-слой (§22.3 п.1) | ❌ |
| 2 | **Consumer billing + кредиты + paywall с дня 1** (подтверждено: Stripe нет) | 3-4 дня | Stripe subscription; 1 разбор ролика = 1 кредит (§22.1 п.8, §22.3 п.2) | ❌ |
| 3 | Self-serve онбординг (email/OAuth, не clinic-cookie) | входит в п.2 | Supabase auth consumer-режим | ❌ |
| 4 | Отдельный лендинг Studio + бренд/домен (обещание 23.1 п.1 масс-маркет) | 1-2 дня | зависит от §22.6 (имя standalone не выбрано) | ❌ |
| 5 | Многоплатформенный постинг (чтобы «→ опубликовано» в демо было настоящим) | этапами | §22.4: v1 share sheet → v2 агрегатор | ❌ |

### 23.4 Формат рекламы (reuse Marketing 01)

Формула из референса — переиспользовать для обоих роликов (детали в `../Marketing 01 — transcript.md`):
1. Один персонаж, один сквозной флоу (сырьё → опубликовано).
2. Каждая фича = конкретная боль своей фразой из жизни.
3. Персона-callout в середине («This is for ___»).
4. Финал = страховка (compliance-гейт / «ничего не утечёт») + онбординг/поддержка.
5. Голос ведёт как оператора; продукт сам себя показывает (запись экрана money-shot, п.23.2.2). Хронометраж ~90-110с.

### 23.5 Рекомендация по последовательности (dual advice)

- **Совет от Crew:** клиника first (выручка + ров), сшить money-shot и снять ролик по §23.4. Studio second.
- **Мой личный совет:** та же очередь, но **лендинг (23.2.1) — раньше seam (23.2.2)**: сначала проверить сообщение на живых клиниках, потом допиливать швы. Порядок Track A: **1 → 2 → 3 → 4 → (5 отложить)**.

### 23.6 Что НЕ решено

- Цена подписки клиники и размер/цена кредитного пакета Studio.
- Бренд/домен Studio (тянется из §22.6).
- Источник метрик для аналитической петли (23.2 п.5): ручной ввод vs интеграция с Meta/аналитикой — решить, когда появятся платящие.
- Канал лидов с лендинга (форма → куда: CRM/GHL/email).

### Стартовый промпт для следующей сессии

> Возвращаюсь в Content Machine, полоса GTM (§23). Клиника-first. Возьми следующий ❌ из §23.2 в порядке 1→2→3→4. Дизайн — через ui-ux-pro-max + HWC бренд-система (Playfair+Inter, ocean/teal/coral). Лендинг — новый route-group `app/(marketing)/`, `app/page.tsx` (логин) не трогать. Перед правкой общих файлов — `git status` (§22.5). По завершении обнови статус пункта.

*Раздел создан: 2026-07-06*
