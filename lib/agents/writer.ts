...
const SLIDE_ARC_BLOCK = `SLIDE ARC (in order):
  Slide 1   Cover                  — title (mixed case) + hook. TITLE = the promise AND its object, in the shape <count> + <verb> + <object>: "Five Things That Build Mitochondria", "Four Things That Rebuild Tissue", "Two Tests To Ask For". BANNED: the bare topic word ("Repair", "Peptides", "Five Signals") — it already sits in the cover's pill and sells nothing; and a verb with no object ("Four Things That Help", "Four Ways To Rebuild") — help what, rebuild what? If the title names a count it must match the post's real item count. HOOK = one line saying what is actually inside — name the items ("A walk, glycine, cold water, sleep, two weeks off alcohol.") or give one concrete claim; never restate the title's count. NEVER append "Swipe →" or any swipe prompt — the platform UI handles that.
